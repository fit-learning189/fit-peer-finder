...
.catch(err =>console.error("Error fetching leaderboard", err));
 ...
 <h2 style={{color: colors.text.hi, fontSize: '2.5rem', marginBottom: '10px'}}>Community Support Stars</h2>
 ...
 <div style={{...styles.podiumName, color: colors.accent.limeHi}}>{leaders[0].name}</div>
 <div style={styles.podiumScore}>{leaders[0].score} pts</div>
 <div style={{...styles.podiumPillar, height: '120px', background: `linear-gradient(to top, ${colors.accent.lime}, ${colors.accent.limeHi})`, border: `1px solid ${colors.accent.limeHi}`, color: colors.text.onAccent}}>1st</div>
 </motion.div>
        )}
        {leaders[2] && (
 <motion.div initial={{ y: 50, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} style={styles.podiumBlockWrap}>
...
const LandingPage = () => {
 ...
 <h1 style={styles.heroTitle}>Learn better, <span style={{color: colors.accent.azureHi}}>together.</span></h1>
 ...
 <motion.div style={{...styles.modalCard, maxWidth: step === 1? '1000px': '600px'}} onClick={e =>e.stopPropagation()}>
              ...
              {step === 1 && (
 <>
 <h2 style={{color: colors.text.hi, marginBottom: '30px'}}>First, select your program</h2>
 ...
 <h2 style={{color: colors.text.hi}}>Which Short Course?</h2>
 <div style={styles.courseList}>
                    {selectedProgram.courses.map(c => (
 <motion.button key={c} whileHover={{ background: colors.accent.azure, color: 'white' }} style={styles.courseBtn} onClick={() => { setSelectedCourse(c); setStep(3); }}>{c}</motion.button>
                    ))}
 </div>
 </>
              )}
 ...
 <h2 style={{color: colors.text.hi}}>How can we connect you?</h2>
 <div style={styles.typeGrid}>
 <OptionCard title="Study Buddy" desc="(1-on-1 Partner)" color={colors.accent.azure} onClick={() =>handleOptionSelect('find')} />
 ...
 <div style={styles.modalCard} onClick={e =>e.stopPropagation()}>
            {feedbackSent? <h3 style={{color: colors.accent.limeHi}}>Thank you!</h3>: (
 <>
 <h3>Rate your experience</h3>
 <div style={{display:'flex', justifyContent:'center', fontSize:'2rem', cursor:'pointer', margin:'10px 0'}}>
                  {[1,2,3,4,5].map(s => (
                    <span key={s} onClick={() =>setRating(s)} style={{display:'inline-flex'}}>
...
// Rating star rendered as inline SVG rather than an emoji glyph, so it
// inherits brand colour and stays visually consistent across platforms.
const StarIcon = ({ filled }) => (
  <svg width="30" height="30" viewBox="0 0 24 24"
       fill={filled ? colors.accent.limeHi : 'none'}
       stroke={filled ? colors.accent.limeHi : colors.text.mute}
       strokeWidth="1.5" strokeLinejoin="round"
       style={{ cursor: 'pointer', transition: 'all 0.15s ease' }}>
    <path d="M12 2.6l2.9 5.88 6.5.95-4.7 4.58 1.11 6.47L12 17.43l-5.81 3.05L7.3 14.01 2.6 9.43l6.5-.95L12 2.6z" />
  </svg>
);
...
const styles = {
  ...
  dropdownMenu: { position: 'absolute', top: '50px', right: '0', background: colors.surface.raised, borderRadius: '8px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', width: '200px', zIndex: 100 },
  dropdownItem: { padding: '15px', cursor: 'pointer', textAlign: 'center', color: colors.accent.azure, fontWeight: 'bold' },
  ...
  primaryBtn: { padding: '12px 18px', borderRadius: '30px', border: 'none', background: colors.accent.azureHi, color: colors.text.hi, fontSize: '0.95rem', fontWeight: 'bold', cursor: 'pointer' },
  primaryBtnFull: { width: '100%', padding: '12px 30px', borderRadius: '30px', border: 'none', background: colors.accent.azureHi, color: colors.text.hi, fontSize: '1rem', fontWeight: 'bold', cursor: 'pointer' },
  secondaryBtn: { padding: '12px 18px', borderRadius: '30px', border: `1px solid ${colors.line.base}`, background: 'transparent', color: 'white', fontSize: '0.95rem', fontWeight: 'bold', cursor: 'pointer' },
  feedbackActionBtn: { padding: '12px 18px', borderRadius: '30px', border: 'none', background: colors.accent.lime, color: colors.text.hi, fontSize: '0.95rem', fontWeight: 'bold', cursor: 'pointer' },
  ...
  modalCard: { background: colors.surface.raised, padding: '2.5rem', borderRadius: '20px', width: '90%', maxHeight: '90vh', overflowY: 'auto' },
  programGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', textAlign: 'left' },
  programCard: { padding: '24px', borderRadius: '14px', border: `2px solid ${colors.line.base}`, background: colors.accent.azureSoft, textAlign: 'left', cursor: 'pointer' },
  programCardName: { fontSize: '1.15rem', fontWeight: '700', color: colors.text.hi, marginBottom: '6px' },
  programCardMeta: { fontSize: '0.85rem', color: colors.text.mute, fontWeight: '500' },
  courseList: { display: 'flex', flexDirection: 'column', gap: '8px' },
  courseBtn: { padding: '12px', borderRadius: '10px', border: `1px solid ${colors.line.base}`, background: colors.surface.raised, textAlign: 'left', cursor: 'pointer', color: colors.text.hi, fontWeight: '500' },
  ...
  leaderboardSection: { padding: '4rem 2rem', background: colors.surface.raised, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  ...
  podiumName: { fontWeight: 'bold', fontSize: '1.1rem', marginBottom: '5px' }, podiumScore: { fontSize: '0.9rem', color: colors.text.lo, marginBottom: '10px' }, podiumPillar: { width: '100%', borderRadius: '10px 10px 0 0', display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '1.5rem', fontWeight: 'bold', color: colors.text.lo }
};

...
const StatusPage = () => {
 ...
 <h3 style={{ color: colors.text.hi, marginTop: '20px' }}>Loading Dashboard...</h3>
 <p style={{ color: colors.text.lo }}>Fetching your latest profile and matching data.</p>
 </div>
 </div>
    );
  }
 ...
 <strong>You recently tried to re-register!</strong><br/>
 ...
 <h4 style={{margin: '0 0 5px 0', color: colors.text.hi}}>Volunteer Status</h4>
                          {remaining > 0? (
 <p style={{margin: 0, color: colors.accent.azure}}>
 <strong>{currentPeersCount}</strong>peer(s) paired. <br/>
 <strong>{remaining}</strong>spots left.
 </p>
                          ): (
 <p style={{margin: 0, color: colors.accent.limeHi}}><strong>Your group is full!</strong></p>
                          )}
 </div>
                  )}
                  {isNeeder && (
 <div style={{...styles.customMsgBox, background: colors.surface.base, border: '1px solid #c8e6c9'}}>
 ...
 <div key={peerIdx} style={{ background: colors.surface.raised, padding: '15px', borderRadius: '10px', marginBottom: '10px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', borderLeft: `5px solid ${colors.accent.azureHi}` }}>
 ...
 <div style={{ marginTop: '20px', background: colors.surface.base, padding: '15px', borderRadius: '10px', textAlign: 'center', border: '1px solid #b8daff' }}>
 ...
 <a href={`https://meet.jit.si/ALX-PeerFinder-${req.group_id}`} target="_blank" rel="noreferrer" style={{ background: colors.accent.azure, color: 'white', padding: '10px 20px', borderRadius: '30px', textDecoration: 'none', fontWeight: 'bold', fontSize: '0.9rem', display: 'inline-block', marginTop: '5px' }}>Join Meeting</a>
 </div>
 ...
 <div style={{...styles.header, background: colors.accent.azureHi}}>
 <p style={{ margin: '0', fontSize: '1.1rem', fontWeight: 'bold' }}>{req.user.program}</p>
 <p style={{ margin: '5px 0 0 0', opacity: 0.9 }}>{req.user.course}</p>
 </div>
 ...
 <div style={{ marginTop: '20px', borderTop: `1px solid ${colors.line.soft}`, paddingTop: '15px' }}>
 <button onClick={() => { setUnpairAction('delete'); setUnpairModal({ isOpen: true, reqId: req.real_id, isMatched: false }); }} style={{...styles.unpairBtn, background: 'transparent', border: `1px solid ${colors.accent.danger}`, color: colors.accent.danger}}>Cancel Request</button>
 </div>
 </div>
 </div>
 </motion.div>
 ...
 <h3 style={{ marginTop: 0, color: colors.text.hi }}>{unpairAction === 'delete'? 'Delete Request': 'Unpair Confirmation'}</h3>

              {unpairModal.isMatched && (
 <p style={{ color: colors.text.lo, fontSize: '0.95rem' }}>Are you sure you want to unpair? Please let your partner(s) know first to be courteous.</p>
              )}

 <div style={{ textAlign: 'left', marginBottom: '20px' }}>
 <label style={{ fontWeight: 'bold', fontSize: '0.9rem', color: colors.text.lo }}>Reason for leaving:</label>
 <select style={{ width: '100%', padding: '10px', marginTop: '5px', borderRadius: '5px', border: `1px solid ${colors.line.base}` }} onChange={(e) =>setUnpairReason(e.target.value)} value={unpairReason}>
 ...
 <input type="email" placeholder="Ghoster's FIT Email" value={ghosterEmail} onChange={(e) =>setGhosterEmail(e.target.value)} style={{width: '100%', padding: '10px', borderRadius: '5px', border: `1px solid ${colors.line.soft}`, boxSizing: 'border-box'}} required />
 </motion.div>
                )}
                {unpairReason === 'Other' && (
 <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} style={{marginTop: '10px', textAlign: 'left', overflow: 'hidden'}}>
 <label style={{fontSize: '0.85rem', fontWeight: 'bold', color: colors.text.lo, display: 'block', marginBottom: '5px'}}>Please specify your reason:</label>
 <input type="text" placeholder="Type your reason here..." value={customReason} onChange={(e) =>setCustomReason(e.target.value)} style={{width: '100%', padding: '10px', borderRadius: '5px', border: `1px solid ${colors.line.base}`, boxSizing: 'border-box'}} required />
 </motion.div>
                )}
 </AnimatePresence>

              {unpairModal.isMatched && (
 <div style={{ textAlign: 'left', marginBottom: '20px', padding: '10px', background: colors.surface.base, borderRadius: '5px', marginTop: '15px' }}>
 <label style={{ fontWeight: 'bold', fontSize: '0.9rem', color: colors.text.lo }}>What happens next?</label>
 ...
 <span style={{ fontSize: '0.9rem', color: colors.accent.danger }}>Delete my profile (I no longer need support)</span>
 </label>
 </div>
 </div>
              )}
 ...
 <button onClick={() =>setUnpairModal({ isOpen: false, reqId: null, isMatched: false })} style={{ flex: 1, padding: '10px', background: colors.surface.high, color: colors.text.lo, border: `1px solid ${colors.line.base}`, borderRadius: '5px', cursor: 'pointer' }} disabled={loadingUnpair}>Cancel</button>
 <button onClick={submitUnpair} style={{ flex: 1, padding: '10px', background: colors.accent.danger, color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold', display: 'flex', justifyContent: 'center' }} disabled={loadingUnpair}>
 ...
 <h2 style={{ color: colors.text.hi, margin: '0 0 10px 0' }}>{feedbackModal.title}</h2>
 <p style={{ color: colors.text.lo, marginBottom: '20px' }}>{feedbackModal.message}</p>
...
const styles = {
  ...
  columnTitle: { color: 'white', borderBottom: `1px solid ${colors.line.base}`, paddingBottom: '10px', margin: '0 0 10px 0', fontSize: '1.2rem' },
  emptyColBox: { background: colors.surface.raised, color: colors.text.lo, padding: '20px', borderRadius: '10px', textAlign: 'center', fontStyle: 'italic', border: `1px dashed ${colors.line.base}` },

  card: { background: colors.surface.raised, width: '100%', borderRadius: '15px', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.3)' },
  header: { background: colors.surface.base, color: 'white', padding: '20px', textAlign: 'center', borderBottom: `4px solid ${colors.accent.limeHi}` },
  body: { padding: '20px' },
  duplicateWarning: { background: colors.surface.base, color: '#856404', padding: '15px', borderRadius: '8px', fontSize: '0.95rem', border: `1px solid ${colors.line.soft}`, textAlign: 'center' },
  ...
  pendingBadge: { background: colors.surface.base, color: '#383d41', padding: '6px 12px', borderRadius: '20px', display: 'inline-block', fontWeight: 'bold', fontSize: '0.8rem', marginBottom: '15px' },
  customMsgBox: { background: colors.surface.base, border: `1px solid ${colors.line.base}`, padding: '12px', borderRadius: '8px', marginBottom: '15px' },

  homeBtn: { padding: '12px 24px', background: 'transparent', border: `1px solid ${colors.line.base}`, color: 'white', borderRadius: '30px', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.3s' },
  unpairBtn: { width: '100%', padding: '10px', background: 'transparent', border: `1px solid ${colors.accent.danger}`, color: colors.accent.danger, borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' },
  ...
  modalContent: { background: colors.surface.raised, padding: '2rem', borderRadius: '15px', width: '90%', maxWidth: '400px', textAlign: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.2)' },
  modalOkBtn: { padding: '12px 30px', background: colors.accent.azure, color: 'white', border: 'none', borderRadius: '20px', fontWeight: 'bold', cursor: 'pointer' },
};
